...
static sl_se_cmac_streaming_context_t cmac_stream_ctx;

/// GCM streaming context
static sl_se_gcm_streaming_context_t gcm_stream_ctx;
...
sl_status_t start_cmac_stream(void)
{
  ...
  // sl_se_cmac_finish is called).
  print_error_cycle(sl_se_cmac_starts(&cmac_stream_ctx,
                                      &cmd_ctx,
...
{
  print_error_cycle(sl_se_cmac_update(&cmac_stream_ctx,
...
{
  print_error_cycle(sl_se_cmac_finish(&cmac_stream_ctx, cipher_msg_buf), NULL);
}
...
sl_status_t start_gcm_encrypt_stream(void)
{
  ...
  print_error_cycle(sl_se_gcm_starts(&gcm_stream_ctx,
                                     &cmd_ctx,
                                     &symmetric_key_desc,
                                     SL_SE_ENCRYPT,
                                     iv_nonce_buf,
 ...
 ******************************************************************************/
sl_status_t update_gcm_encrypt_stream(uint32_t offset)
{
  print_error_cycle(sl_se_gcm_update(&gcm_stream_ctx,
                                     plain_msg_len,
                                     plain_msg_buf + offset,
                                     cipher_msg_buf + offset), NULL);
}
 ...
 ******************************************************************************/
sl_status_t update_gcm_decrypt_stream(uint32_t offset)
{
  print_error_cycle(sl_se_gcm_update(&gcm_stream_ctx,
                                     plain_msg_len,
                                     cipher_msg_buf + offset,
                                     plain_msg_buf + offset), NULL);
}

/***************************************************************************//**
 * Finish an AES GCM stream.
 ******************************************************************************/
sl_status_t finish_gcm_stream(void)
{
  print_error_cycle(sl_se_gcm_finish(&gcm_stream_ctx,
