...
void app_process_action(void)
{
  switch (app_state) {
    ...
    case UPDATE_GCM_ENCRYPT_STREAM:
      ...
      if (update_gcm_encrypt_stream(AES_BLOCK_SIZE * stream_block_count)
          == SL_STATUS_OK) {
        stream_block_count++;
    ...
    case FINISH_GCM_ENCRYPT_STREAM:
      ...
      if (finish_gcm_stream() == SL_STATUS_OK) {
        ...
        if (memcmp(gcm_cipher_text, get_cipher_msg_buf_ptr(),
            ...
            && memcmp(gcm_tag, get_cipher_msg_buf_ptr() + sizeof(gcm_plain_text),
                      sizeof(gcm_tag)) == 0) {
          printf("OK\n");
    ...
    case UPDATE_GCM_DECRYPT_STREAM:
      printf("  + Decrypting %d bytes ciphertext with %d-bit key... ",
             AES_BLOCK_SIZE, get_symmetric_key_len());
      if (update_gcm_decrypt_stream(AES_BLOCK_SIZE * stream_block_count)
          == SL_STATUS_OK) {
        stream_block_count++;
        stream_msg_size -= AES_BLOCK_SIZE;
        if (stream_msg_size <= AES_BLOCK_SIZE) {
