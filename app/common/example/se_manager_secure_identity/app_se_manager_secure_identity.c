...
/// Challenge buffer
static uint8_t challenge_buf[CHALLENGE_SIZE];

/// Certificate buffer
static uint8_t cert_buf[CERT_SIZE];

/// Public device key buffer
static uint8_t pub_device_key_buf[PUB_KEY_SIZE];
...
sl_status_t sign_challenge(void)
{
  ...
  sl_se_key_descriptor_t private_device_key = {
    ...
    .storage.location.slot = SL_SE_KEY_SLOT_APPLICATION_ATTESTATION_KEY
  };
 ...
 ******************************************************************************/
sl_status_t verify_signature(void)
{
  // Set up a key descriptor for public device key
