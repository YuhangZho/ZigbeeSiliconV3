    ...
    case SELECT_KEY_TYPE:
      if (pb0_press) {
        ...
        printf("  + Current symmetric key is a %s key.\n",
               asymmetric_key_type_string[symmetric_key_type_select]);
      }
      if (pb1_press) {
        pb1_press = false;
        printf("\n  . Current symmetric key length is %lu-bit.\n",
               symmetric_key_size[symmetric_key_size_select] * 8);
        printf("  + Press PB0 to select symmetric key length (128 or 192 or "
               "256), press PB1 to next option.\n");
    ...
    case 2:
      if (create_volatile_key_size != 0xff) {
        printf("  + Delete a non-exportable symmetric key in a volatile SE "
               "key slot... ");
        ret = delete_volatile_asymmetric_key(symmetric_key_size[create_volatile_key_size]);
        if (ret != SL_STATUS_OK) {
          break;
        }
      }
      ...
      printf("  + Generating a %lu-bit non-exportable symmetric volatile key... ",
             symmetric_key_size[symmetric_key_size_select] * 8);
      ret = create_volatile_asymmetric_key(symmetric_key_size[symmetric_key_size_select]);
      break;
...
static void print_key_type(void)
{
  ...
  printf("\n  . Current symmetric key length is %lu-bit.\n",
         symmetric_key_size[symmetric_key_size_select] * 8);
  printf("  + Press PB0 to select symmetric key length (128 or 192 or 256), "
