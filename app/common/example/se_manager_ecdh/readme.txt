...
This example uses the SE Manager API to perform the key agreement (ECDH) in 
...
spent in ECDH operations. The results are printed to stdout, i.e. the VCOM 
serial port console. This feature can be disabled by defining SE_MANAGER_PRINT=0
(default is 1) in IDE setting (Preprocessor->Defined symbols).
...
In this example, two peers (client and server) each generate their own Elliptic 
Curve Cryptography (ECC) key pair. They exchange public keys and compute a 
shared secret using their private ECC keys. These secrets are compared with each
other to make sure they are equal.
