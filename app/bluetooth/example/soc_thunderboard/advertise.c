...
void advertise_init(uint32_t unique_id)
{
  ...
  sl_app_assert(local_name_length == ADVERTISE_DEVICE_NAME_LENGTH,
...
void advertise_start(void)
{
  ...
  sc = sl_simple_timer_start(&adv_timer,
                             ...
                             true);
  sl_app_assert(sc == SL_STATUS_OK,
                "[E: 0x%04x] Failed to start advertising timer\n",
                (int)sc);
}
...
void advertise_stop(void)
{
  ...
  sl_app_assert(sc == SL_STATUS_OK,
                "[E: 0x%04x] Failed to stop advertising timer\n",
                (int)sc);
  // Stop advertising
  sc = sl_bt_advertiser_stop(adv_set_handle);
  sl_app_assert(sc == SL_STATUS_OK,
                "[E: 0x%04x] Failed to stop advertising\n",
                (int)sc);
