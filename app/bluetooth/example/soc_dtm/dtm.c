...
static void process_setup_command(setup_cmd_packet_t *cmd)
{
  ...
  if (cmd->control == SETUP_CMD_RESET) {
    ...
    sl_app_assert(sc == SL_STATUS_OK,
                  "[E: 0x%04x] Failed at test dtm end.\n",
                  (int)sc);
  ...
  switch (cmd->control) {
    ...
    case SETUP_CMD_TRANSMITTER_POWER_LEVEL:
      ...
      if (tx_power == 0x7E) {
        setup.transmitter_power_level = -127;
      } else if (tx_power == 0x7F) {
        setup.transmitter_power_level = 20;
      } else if (tx_power <= 20) {
      ...
      if (status != TEST_STATUS_ERROR) {
        ...
        sc = sl_bt_system_set_max_tx_power(setup.transmitter_power_level, &set_power);
        sl_app_assert(sc == SL_STATUS_OK,
                      "[E: 0x%04x] Failed to set max tx power\n",
                      (int)sc);
        response = (uint8_t)(set_power / 10);
  ...
  switch (cmd_type) {
    case CMD_TYPE_RXTEST:
      ...
      if (setup.cte_present) {
        if (setup.ant_array != 0) {
          ...
          sl_app_assert(sc == SL_STATUS_OK,
                        "[E: 0x%04x] Failed to set cte reciever params.\n",
                        (int)sc);
        }
      } else {
    ...
    case CMD_TYPE_TXTEST:
      if (setup.cte_present) {
        if (setup.ant_array != 0) {
          sc = sl_bt_cte_transmitter_set_dtm_parameters(setup.cte_time, setup.cte_type,
                                                        setup.ant_array_length, setup.ant_array);
          sl_app_assert(sc == SL_STATUS_OK,
                        "[E: 0x%04x] Failed to set cte transmitter params.\n",
                        (int)sc);
        }
      } else {
      ...
      if (PACKET_TYPE_MAX > cmd->pkt) {
        (void)sl_bt_test_dtm_tx(std_to_bgapi_pkt_types[cmd->pkt], length, cmd->frequency, setup.phy);
      }
      break;
