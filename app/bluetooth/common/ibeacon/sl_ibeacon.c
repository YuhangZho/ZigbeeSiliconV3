...
void sl_bt_beacon_on_event(sl_bt_msg_t *evt)
{
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    case sl_bt_evt_system_boot_id:
      sl_app_log("Beacon Bluetooth stack booted: v%d.%d.%d-b%d\n",
      ...
      sc = sl_bt_system_set_max_tx_power(0, &ret_power);
      sl_app_assert(sc == SL_STATUS_OK,
                    "[E: 0x%04x] Failed to set max TX power for Bluetooth\n",
                    (int)sc);
      sl_app_log("Beacon Bluetooth system TX power set to %d\n",
                 (int)ret_power);
      ...
      sl_app_assert(sc == SL_STATUS_OK,
                    ...
                    (int)sc);
      sl_app_log("Beacon Bluetooth address: %02X:%02X:%02X:%02X:%02X:%02X, type: %d\n",
                 address.addr[5],
                 address.addr[4],
                 address.addr[3],
                 address.addr[2],
      ...
      sl_app_log("Beacon started advertising\n");
      break;
  }
}
