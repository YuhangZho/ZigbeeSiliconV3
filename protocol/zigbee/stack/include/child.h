...
/** @brief Defines tasks that are taken into account when dealing with
 *  sleep related behavior.
 *
 *  As opposed to its subset of EMBER_HIGH_PRIORITY_TASKS, these tasks does not
 *  necessarily prevent us to go to sleep. However, they are taken into account
 *  when calculating how long we can sleep until the next event.
 */
enum {
