...
/** @brief Broadcasts a new encryption key but
...
/** @brief Sends a unicast update of the network key
...
/** @brief Sends the network key
 *  to the target device. The APS command will be encrypted using
 *  the device's current APS link key. The call would fail if there is no
 *  link key avaialble.
 *
 *
 *  Both the short and long address of the device must be known ahead of
...
/** @brief Broadcasts a switch key message to
...
/** @brief Callback to allow the application running on the Trust
...
/** @brief Causes a coordinator to become the Trust Center
...
/** @brief Sends an APS remove device command to the destination.
...
/** @brief Sends the trust center link key to a node.
 *
