...
/** @brief Sets the initial security state that will be used by
...
/** @brief Gets the extended security bitmask that is used by a device.
 *
...
/** @brief Gets the security state that is used by a device joined
 *  into the Network.
 *
...
/** @brief Callback to inform the application that the Network Key
...
/** @brief Requests a new link link key from the Trust Center. This
...
/** @brief Callback to notify the application about the status
...
/** @brief Gets data from the Key Table.
 *  On the 35x series chips, the data returned by this call is governed by the
 *  security policy set in the manufacturing token for TOKEN_MFG_SECURITY_CONFIG.
 *  See the API calls ::emberSetMfgSecurityConfig() and ::emberGetMfgSecurityConfig()
 *  for more information.  If the security policy is not set to
...
/** @brief Sets an entry in the key table.
 *
...
/** @brief Adds a new entry in the key table or
...
/** @brief Searches the key table to find an entry
...
/** @brief Clears a single entry in the key table.
 *
...
/**
 * @brief Clears the key table of the current network.
 *
...
/** @brief Suppresses normal write operations of the stack
...
/** @brief Immediately writes the value of stack tokens
 *    and then resumes normal network operation by writing the stack tokens
...
/** @brief Determines whether stack tokens will be written
...
/** @brief Performs APS encryption/decryption of messages
...
/** @brief Retrieves the security configuration stored
...
/** @brief Sets the security configuration to be stored
 *    in manufacturing tokens.  It is only available on the 35x series.
...
/** @brief Sets the NWK layer outgoing frame counter
...
/** @brief Sets the APS layer outgoing frame counter for Trust Center
...
/** @brief Adds a temporary link key for a joining device. The link key will
...
/** @brief Gets the transient link key at the index specified in the transient
 *  key table.
 *
...
/** @brief Gets the transient link key for a device. This function searches the
...
/** @brief Clears all transient link keys from RAM. */
...
/*
 * @brief Returns the current rejoin policy, which can either allow no rejoin
...
/*
 * @brief Sets the current rejoin policy, which can either allow no rejoin
