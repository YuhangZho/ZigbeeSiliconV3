...
class GattXmlParser(object):
    ...
    def parse_gattdb(self, xml):
        ...
        db.header_macro = "__" + db.hfile.replace(".", "_").upper()
        db.is_default = xml.get('default', "true") == "true"
        db.id_prefix = xml.get('prefix', "")
        db.name = xml.get('db_name', "bg_gattdb_data")
    ...
    def parse_characteristic(self, xml,parent_caps):
        ...
        if dn is not None:
            ...
            valid_properties = (d.properties & ~(GattConst.CHR_PROPERTY_READ|GattConst.CHR_PROPERTY_WRITE|GattConst.CHR_PROPERTY_WRITENORESP)) == 0
