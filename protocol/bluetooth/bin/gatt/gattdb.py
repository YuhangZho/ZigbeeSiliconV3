...
class GattDatabase(object):
    ...
    def __init__(self):
        ...
        self.advertised_services_16= []
        self.advertised_services_16_len= 0
        self.advertised_services_128= []
        self.advertised_services_128_len= 0
    ...
    def set_services(self, services,add_generic_attribute_service=True):
        ...
        for s in self.services:
            ...
            if (s.advertise):
                if(s.uuid.isshort):
                    self.advertised_services_16+=(s.uuid.bytes)
                    self.advertised_services_16_len+=1
                else:
                    self.advertised_services_128+=(s.uuid.bytes)
                    self.advertised_services_128_len+=1

        #if empty, add zero values
        if self.advertised_services_128_len==0:
            self.advertised_services_128=[0]
        if self.advertised_services_16_len==0:
            self.advertised_services_16=[0]
        ...
        for a in self.attribute_rows:
            ...
            if a.uuid==BluetoothUuid(GattConst.UUID_INCLUDE_SERVICE):
                ...
                b=bytearray(4)
                b[0] = included.start & 0xff;
                b[1] = included.start >> 8;
                b[2] = included.end & 0xff;
                b[3] = included.end >> 8;
                #only 16bit uuid id is added to attribute
                if(included.uuid.isshort):
                    b+=included.uuid.bytes
                a.value=GattValue(GattConst.ConstValue,b)
    ...
    def __build_atrribute_rows(self):
        ...
        for a in self.attribute_rows:
            ...
            if(a.value!=None and (a.value.type == GattConst.DynamicValue or a.value.type == GattConst.VariableLengthValue or a.value.type == GattConst.UserValue)):
                a.value.dynamic_index=len(self.dynamic_attributes)
                self.dynamic_attributes.append(a)
            if(a.value!=None and ((a.value.type == GattConst.ClientConfigValue)or(a.value.type == GattConst.ClientConfigValue))):
                a.value.dynamic_index=len(self.dynamic_attributes)-1 # Same as previous attribute
                a.value.bytes.append(num_ccfg)
                num_ccfg+=1
    ...
    def __build_service(self, service):
        ...
        service_declare = GattAttribute(uuid=uuid, permissions=GattConst.Read|GattConst.Discoverable, value=value, capabilities=service.capabilities)
    ...
    def __build_characteristic_db(self, ch, start_handle):
        ...
        rows=[]
        data = bytearray()
        data.append(ch.properties&0xff)
        data.append((start_handle+1)&0xff)
        data.append((start_handle+1)>>8)
        for i in ch.uuid.bytes:
            data.append(i)
        value = GattValue(GattConst.ConstValue, data)
        char_declare = GattAttribute(uuid=GattConst.UUID_CHR, value=value, capabilities=ch.capabilities)
        ...
        if (ch.properties & (GattConst.CHR_PROPERTY_INDICATE | GattConst.CHR_PROPERTY_NOTIFY) and ch.get_descriptor(GattConst.UUID_CHR_CLIENT_CONFIG)==None):
            ...
            permissions=GattConst.Write|GattConst.WriteNoResp|GattConst.Read|GattConst.Discoverable
        ...
        if (ch.properties & GattConst.CHR_PROPERTY_BROADCAST):
            permissions=GattConst.Write|GattConst.WriteNoResp
