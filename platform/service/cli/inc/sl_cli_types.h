...
#endif
#if defined(SL_CATALOG_KERNEL_PRESENT)
  uint32_t start_delay_ms;                     ///< A delay after the CLI task has started before any actions.
  uint32_t loop_delay_ms;                      ///< A delay in the CLI task loop.
#else
...
typedef struct {
  ...
  uint32_t prio;                                 ///< The task priority.
