...
# Design
To fully understand how to use the CLI, it is crucial to understand the design and some implementation deatils. The following sections will try to explain the basics and how to create user commands.

## Modules
The CLI has been implementated as two independent software modules; `input handler` and `command interpreter`. The `input handler` handles the user input and allows the user to enter command lines. When the user type the ENTER key, the input string is passed to the `command interpreter` where the string will be interpreted and the appropriate command handler will be called with the command arguments.
...
The components come with the command handlers and do all the necessary integration with the CLI. The componentes do however not define the command name, it must be defined by the application implementer.
...
## CLI Storage NVM3
The `cli_storage_nvm3` component has been designed to store command lines in NVM3 storage. In addition to let the user control when the commands are executed, the stored commands will automatically be executed at program start.
