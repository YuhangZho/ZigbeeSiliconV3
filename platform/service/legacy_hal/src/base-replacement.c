...
// The heapMemory here is defined in ember-configuaration.c in protocol/flex
// This needs to be revisited while working on Zigbee on UC.
...
EmberStatus emberSerialPrintfLine(uint8_t port, const char * formatString, ...)
{
  ...
  emberSerialPrintfVarArg(0, formatString, ap);
  va_end(ap);
  printf("\n");
  return EMBER_SUCCESS;
}
