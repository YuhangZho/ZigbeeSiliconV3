...
class CALC_Demodulator_ocelot(ICalculator):
    ...
    def calc_src2_dec2(self,model):
        ...
        if model.vars.MODEM_CF_DEC2.value_forced != None and model.vars.MODEM_SRCCHF_SRCRATIO2.value_forced != None:
            model.vars.dec2.value = model.vars.MODEM_CF_DEC2.value + 1
            model.vars.src2_ratio.value = model.vars.MODEM_SRCCHF_SRCRATIO2.value / 16384.0
            return
        elif model.vars.MODEM_CF_DEC2.value_forced != None:
            model.vars.dec2.value = model.vars.MODEM_CF_DEC2.value + 1
            model.vars.src2_ratio.value = target_osr * 8.0 * dec0 * dec1 * model.vars.dec2.value * baudrate / adc_freq
            return
        elif model.vars.MODEM_SRCCHF_SRCRATIO2.value_forced != None:
            model.vars.src2_ratio.value = model.vars.MODEM_SRCCHF_SRCRATIO2.value / 16384.0
    ...
    def calc_src2_reg(self,model):
        ...
        if model.vars.MODEM_SRCCHF_SRCRATIO2.value_forced != None:
            src2_forced =  model.vars.MODEM_SRCCHF_SRCRATIO2.value / 16384.0
    ...
    def calc_lock_bandwidth(self,model):
        ...
        elif (afc_run_mode == model.vars.afc_run_mode.var_enum.ONE_SHOT):
...
#TODO: Go through these and decide if they still apply for Ocelot
    ...
    def calc_src_disable(self,model):
        #This function sets a default value for src_disable
        model.vars.src_disable.value = model.vars.src_disable.var_enum.ENABLED

    def calc_dsa_enable(self,model):
        #This function sets a default value for dsa_enable
    ...
    def calc_src2_denominator(self, model):
        ...
        if (model.vars.adc_clock_mode.var_enum.HFXOMULT == adc_clock_mode):
            ...
            src2_calcDenominator = datarate * dec0 * dec1 * 8 * osr
    ...
    def return_osr_dec0_dec1(self, model, demod_select, withremod=False, relaxsrc2=False):
        ...
        if demod_select == model.vars.demod_select.var_enum.BCR:
            ...
            target_bwsel = 0.3
            max_bwsel = 0.3
        elif demod_select == model.vars.demod_select.var_enum.OFDM:
            #FIXME SOL: update these ranges for OFDM modem
            osr_list = [4]
            min_osr = 4
            max_osr = 4
            min_src2 = 0.6
        ...
        if model.vars.MODEM_CF_DEC0.value_forced != None:
            dec0_forced = self.return_dec0_from_reg(model.vars.MODEM_CF_DEC0.value)
            dec0_list = [ dec0_forced ]
        else:
            # calculate prioritized list for dec0
        ...
        for osr in osr_list:
            for dec0 in dec0_list:
                ...
                if model.vars.dec1._value_forced != None:
                    dec1_forced = model.vars.MODEM_CF_DEC1.value + 1
                    dec1_list = [dec1_forced]
                else:
                    min_dec1 = int(max(1, ceil(float(adc_freq) * min_bwsel / (8 * dec0 * bandwidth))))
                    max_dec1 = int(min(65535, floor(float(adc_freq) * max_bwsel / (8 * dec0 * bandwidth))))
                ...
                for dec1 in dec1_list:
                    ...
                    if demod_select == model.vars.demod_select.var_enum.BCR:
                        ...
                        # forcing integer osr condition, which applies to TRECS, LEGACY, COHERENT
                    ...
                    forced_dec2_out_of_range = 0
                    if model.vars.MODEM_CF_DEC2.value_forced != None:
                        dec2_forced = model.vars.MODEM_CF_DEC2.value_forced + 1
                        if (dec2_forced < calc_min_dec2) or (dec2_forced > calc_max_dec2):
                            forced_dec2_out_of_range = 1

                    if (calc_min_dec2 <= calc_max_dec2) and (calc_min_dec2 <= max_dec2) and \
                            (calc_max_dec2 >= min_dec2) and (forced_dec2_out_of_range == 0):
