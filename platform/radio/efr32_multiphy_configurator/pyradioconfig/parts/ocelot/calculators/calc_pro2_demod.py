...
class CALC_Pro2_Demod_Ocelot(ICalculator):
    ...
    def calc_run_pro2(self,model):
        ...
        #Create the input dict
            ...
            pro2_inputs["API_freq_xo"] = xtal_frequency_hz
            ...
            pro2_inputs["API_Manchester"] = (symbol_encoding == model.vars.symbol_encoding.var_enum.Manchester)
            ...
            pro2_inputs["API_pm_len"] = model.vars.preamble_length.value_forced
    ...
    def _map_pro2_outputs(self,model, pro2_calculator_obj):
        ...
        self._reg_write(model.vars.MODEM_BCRDEMODOOK_ABPK, 0)
        self._reg_write(model.vars.MODEM_BCRDEMODOOK_NOISEFLEST, 0)
        ...
        self._reg_write(model.vars.MODEM_BCRDEMODARR0_PHSPIKETHD, int(pro2_calculator_obj.demodulator.fields.spike_det_thd))
        ...
        self._reg_write(model.vars.MODEM_BCRDEMODARR1_ENCFEQUAL, pro2_calculator_obj.demodulator.fields.signal_dsa_mode)
    ...
    def calc_pro2_dsa_mode(self,model):
        ...
        if (mod_type == 1):  ## OOK or ASK
            if (preamble_length >= 16):
                pro2_dsa_mode = 0
            else:
                pro2_dsa_mode = 1
        else:
            if (16 <= preamble_length < 48) :
                pro2_dsa_mode = 1
            else:
                pro2_dsa_mode = 0
    ...
    def calc_pro2_afc_mode(self,model):
        ...
        #     pro2_afc_mode = 1
        if (16 <= preamble_length < 48) and ( mod_type!= 1) :
    ...
    def calc_bcr_manchph(self,model):
        ...
        if (symbol_encoding == model.vars.symbol_encoding.var_enum.Manchester):
            if (manchester_mapping == model.vars.manchester_mapping.var_enum.Inverted) and (demod_select == model.vars.demod_select.var_enum.BCR):
                manchph = 1
            else:
                manchph = 0
        else:
    ...
    def calc_bcr_cfe_costthd(self,model):
        ...
        if model.vars.bcr_demod_en.value == 1:
            ...
            self._reg_sat_write(model.vars.MODEM_BCRDEMODPMEXP_BCRCFECOSTTHD, bcr_cfe_costthd, model.part_family)
        else:
    ...
    def calc_bcr_cfeschwin(self, model):
        ...
        if preamble_len < 32: # : for short preamble, keep CFE window short
            bcrcfeschwin = 3
        elif 32 <= preamble_len < 40:
    ...
    def calc_bcr_cfequalthd(self, model):
        ...
        if model.vars.bcr_demod_en.value == 1:
            ...
            self._reg_write(model.vars.MODEM_BCRDEMODARR1_CFEQUALTHD, cfequalthd)
        else:
            self._reg_do_not_care(model.vars.MODEM_BCRDEMODARR1_CFEQUALTHD)

    def calc_bcr_pmacqwin(self, model):
        preamble_len = model.vars.preamble_length.value
        estosren_actual = model.vars.MODEM_BCRCTRL1_ESTOSREN.value

        if preamble_len < 32: # : for short preamble, keep preamble  window short
            pmacqwin = 2
        elif 32 <= preamble_len < 40:
            if estosren_actual == 1:
                pmacqwin = 2
