...
class CALC_Frame(ICalculator):
    ...
    def buildVariables(self, model):
        ...
        self._addModelVariable(model, 'firstframe_bitsperword', int, ModelVariableFormat.DECIMAL, units='bytes', desc='First word in a frame contains (BITSPERWORD+1) bits, while all following words always contain 8 bits. In most cases, set to 7.')
    ...
    def calc_frame(self, model):
        ...
        model.vars.firstframe_bitsperword.value = (8 - 1)

        # Unless specied via advanced input
        firstframe_bitsperword = model.vars.firstframe_bitsperword.value
        model.vars.firstframe_bitsperword.value = firstframe_bitsperword


        self._reg_write(model.vars.FRC_CTRL_BITSPERWORD, model.vars.firstframe_bitsperword.value)
