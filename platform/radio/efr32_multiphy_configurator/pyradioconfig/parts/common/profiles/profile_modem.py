...
def buildModemAdvancedInputs(model, profile, family):
    ...
    if family == "lynx":
        IProfile.make_hidden_input(profile, model.vars.enable_retime                     , 'Advanced', readable_name="Enable retiming",                              value_limit_min=0,      value_limit_max=3)
    ...
    IProfile.make_linked_io(profile, model.vars.firstframe_bitsperword, 'Advanced', readable_name="First word in frame contains (BITSPERWORD+1) bits. In most cases, set to 7.",                               value_limit_min=0,     value_limit_max=7)
