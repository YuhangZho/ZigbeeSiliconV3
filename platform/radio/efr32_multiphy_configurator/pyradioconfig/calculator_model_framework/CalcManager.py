...
class CalcManager(object):
    ...
    def _getCalculatorsList(self):
        ...
        try:
            ...
            if part_family.lower() in ['dumbo', 'jumbo', 'lynx', 'nerio', 'nixi', 'panther', 'unit_test_part']:
    ...
    def calculate_phy(self, phy_name=None, optional_inputs=None):
        ...
        if not self.check_phy_supported_on_target(phy_name):
    ...
    def calc_config(self, phy_name, output_path, optional_inputs=None, show_do_not_care=True):
        ...
        if model_instance.result_code != CalcStatus.Failure.value:
            ...
            self.processed_model_to_cfg(model_instance, output_path, phy_name, show_do_not_care=show_do_not_care)
    ...
    def processed_model_to_cfg(self, model_instance, output_file, phy_name=None, show_do_not_care=True):
        ...
        Human_Readable.print_modem_model_values_v2(output_path_cfg, phy_name, model_instance, show_do_not_care=show_do_not_care)
    ...
    def check_phy_supported_on_target(self, phy_name):
        target_tag = self.getTargetTag()
        blacklist_tag = "-"+target_tag
        target_blacklisted = self.phy_has_tag(phy_name,blacklist_tag)
