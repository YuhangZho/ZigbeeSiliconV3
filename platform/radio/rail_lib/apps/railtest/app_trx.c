...
void RAILCb_TxPacketSent(RAIL_Handle_t railHandle, bool isAck)
{
  ...
  if (isAck) {
    ...
    if (txTimePosition == RAIL_PACKET_TIME_DEFAULT) {
      previousTxAckAppendedInfo.timeSent.totalPacketBytes = ackDataLen;
      (void) RAIL_GetTxPacketDetailsAlt2(railHandle, &previousTxAckAppendedInfo);
      (void) RAIL_GetTxTimeFrameEndAlt(railHandle, &previousTxAckAppendedInfo);
    } else {
    ...
    if (txTimePosition == RAIL_PACKET_TIME_DEFAULT) {
      ...
      (void) RAIL_GetTxTimeFrameEndAlt(railHandle, &previousTxAppendedInfo);
    } else {
